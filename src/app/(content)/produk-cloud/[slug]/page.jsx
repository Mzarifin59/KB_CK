...
export async function generateMetadata({params}){
  ...
  return{
    title: metaData[0]?.text || 'Produk Cloud',
    description: metaData[0]?.description || 'Produk Cloudkilat yang menjadi produk utama cloud',
  }
};

export default async function KilatStoragePage({params}){
  const category = await getCategoryDynamic(params.slug)

  return <KilatStorage category={category} slug={params.slug}/>
}

function KilatStorage({ slug, category }) {
  return (
    <>
                      ...
                      <>
                        {child.articles.map((link) => (
                          <>
                            <Link
                              key={link.documentId}
                              href={`/produk-cloud/${slug}/${child.slug}/${link.slug}`}
